...
- необязательные подсказки `Поколение` и `Порядок в поколении` для сложных
  родословных с несколькими параллельными семьями;
- отдельный перенос длинных имён по более узкому лимиту и увеличенный
  вертикальный интервал между поколениями для плотных книжных схем;
...
| № | Имя | Титул | Начало правления | Конец правления | Дети | Брак | Поколение | Порядок в поколении |
|---|---|---|---|---|---|---|---|---|
...
Поэтому все заполненные значения в колонке `№` должны быть уникальны в пределах
листа. При повторе launcher показывает обе Excel-ячейки и имена конфликтующих
людей. Красивое отображаемое название не влияет на связи.

Несколько детей, супругов или титулов разделяются точкой с запятой либо переносом
строки. Титулы автоматически приводятся к единому написанию со строчной буквы.
Запятая после имени формируется самой программой и зависит от содержимого графы
`Титул`: она ставится только перед настоящим текстовым титулом. Ручную запятую в
графе `Имя` добавлять не требуется; старые ручные запятые программа сначала удаляет.

```text
Оттон I + король Восточных Франков (936-973)
→ Оттон I, король Восточных Франков (936-973)
...
Несколько упорядоченных ссылок в графах `Дети` и `Брак` можно записывать так:
...
конечных годов. Если правление продолжается до конца жизни или конечный год не
нужно указывать, соответствующую позицию в `Конец правления` оставляют пустой:

```text
Титул:             Король; Император
Начало правления:  936; 962
Конец правления:   973;
Результат:          (936-973, император с 962)
```

Полные периоды правления всегда выводятся раньше дополнительных уточнений
вида `с ГОДА`, даже если в Excel титулы перечислены в обратном порядке.
Например, `Император; Король` с периодами `962; 936` и `;973` всё равно будет
отображён как `(936-973, император с 962)`.

Супружеская связь на схеме отображается компактным традиционным знаком `=` —
двумя короткими параллельными линиями по центру промежутка между супругами.
Линия потомков выходит из нижней линии этого знака.

### Управление сложной раскладкой

Обе дополнительные колонки необязательны. Старые книги без них продолжают
работать в полностью автоматическом режиме.

- `Поколение` — положительное целое число, начиная с `1`. Значение может
  отражать реальную глубину человека в конкретной родовой ветви, поэтому у
  супругов оно вправе различаться. Партнёрский блок всегда рисуется на одном
  горизонтальном ряду — для него выбирается самое глубокое из явно заданных
  поколений. Пустое значение вычисляется автоматически по связям родитель →
  ребёнок.
- `Порядок в поколении` — относительный порядок слева направо. Удобно задавать
  значения с шагом 10 (`10`, `20`, `30`), чтобы позднее вставить человека между
  существующими позициями. Это не координата: реальные расстояния по-прежнему
  рассчитываются автоматически с учётом ширины подписей и семейных связей. В
  межпоколенческом браке порядок общего блока берётся у человека из самого
  глубокого явно заданного поколения; порядок супруга из более раннего ряда не
  создаёт ложного конфликта на новом визуальном уровне.

Если порядок заполнен только у части людей, он действует как частичное
ограничение: пронумерованные семейные блоки меняются местами между собой, а
непронумерованные сохраняют автоматический порядок. Для полного контроля над
конкретной горизонтальной линией следует заполнить порядок у всех её людей.

Программа явно отклоняет действительно противоречивые данные: выбранный для
семейного блока ряд выше одного из его родителей и одинаковый явный порядок у
разных семейных блоков одного поколения. Разные поколения у супругов допустимы:
вся пара переносится на более глубокий ряд.
...
- разрешение неоднозначных имён опирается на имя, титул, уточнение и контекст
  строк Excel; для визуального порядка сложных схем следует использовать
  отдельные колонки раскладки, а не переставлять строки ради renderer;
