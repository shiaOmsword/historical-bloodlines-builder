...
установка не требуется. Сборщик автоматически определяет фактический каталог
plug-in DLL, потому что разные Windows-дистрибутивы Graphviz используют разную
структуру. После распаковки приложение проверяет реальный рендер `neato -> SVG` и
при необходимости обновляет конфигурацию plug-inов. Поэтому portable-архив нужно
распаковывать в обычную папку, доступную пользователю для записи.

Пользователь выбирает `.xlsx` и место сохранения через стандартные окна Windows.
После построения launcher предлагает открыть результат или показать его в
Проводнике. Launcher также включает UTF-8 и Virtual Terminal Processing в
Windows-консоли, а ширина панелей подстраивается под текущее окно.
...
2. запускает тесты;
3. копирует полную Windows-установку Graphviz в `vendor/graphviz`, выполняет
   `dot -c` и проверяет plug-in `neato`;
