...
- `Поколение` — положительное целое число, начиная с `1`. Достаточно указать его
  у одного участника партнёрского блока; значение применяется ко всему блоку.
  Если поколение заполнено у нескольких супругов, значения должны совпадать.
  Пустое значение вычисляется автоматически по связям родитель → ребёнок.
- `Порядок в поколении` — относительный порядок слева направо. Удобно задавать
  ...
  рассчитываются автоматически с учётом ширины подписей и семейных связей.
...
Программа явно отклоняет противоречивые данные: разные поколения у супругов,
поколение ребёнка не ниже родителей и одинаковый явный порядок у разных
семейных блоков одного поколения.
